/* ===== 金のフレーズ 單字 App — Service Worker（離線快取）=====
   - 頁面與單字資料：網路優先，離線時退回快取（每次上線自動更新）
   - 音檔（.mp3）：快取優先；支援 Range 請求，Safari 也能離線播放
   - 音檔快取名稱與 index.html 內的下載管理共用：kin-audio-v1
*/
<<<<<<< HEAD
var VER = 'kin-shell-v5';
...
>>>>>>> d18fa7b3e10932aed747fcd44c976ef781b419b0
  ...
  ev.waitUntil(
    caches.keys().then(function(keys){
      return Promise.all(keys.map(function(k){
        if(k !== VER && k !== AUDIO) return caches.delete(k);
      }));
...
/* 從完整快取回應切出 206 Partial Content，供 <audio> 的 Range 請求使用 */
...
self.addEventListener('fetch', function(ev){
  ...
  if(url.origin !== self.location.origin) return;   // Google Fonts、GitHub API 等交給瀏覽器

  if(isAudio(url)){
    ev.respondWith(
      caches.open(AUDIO).then(function(cache){
        var key = new Request(url.href);          // 以網址為鍵，忽略 Range 標頭
        return cache.match(key).then(function(hit){
          if(hit) return rangeResponse(req, hit.clone());
          // 未快取：以完整請求取回，存一份，再依 Range 回應
